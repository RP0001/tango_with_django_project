...
def populate():
    ...
    django_pages = [
        ...
        "url" :"https://docs.djangoproject.com/en/1.9/intro/tutorial01/", "views": 4},
        {"title" : "Django Rocks",
...
def add_page(cat, title, url, views=0):
    p = Page.objects.get_or_create(category=cat, title=title)[0] #get_or_create checks if something already exists
    p.url = url
    p.views = views
    p.save()
    return p
...
# Start execution here! the __main__ trick here is really like a java main class with psvm, importing module does not cause this
